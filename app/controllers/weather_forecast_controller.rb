...
class WeatherForecastController < ApplicationController
  ...
  def get_location_details(address)
    ...
    if location_co_ordinates.present?
      return { lat: location_co_ordinates&.data&.[]('lat'),
               lon: location_co_ordinates&.data&.[]('lon') }
    end
  ...
  def get_weather_forecast(latitude, longitude)
    ...
    variables = { current: %i[temperature_2m windspeed_10m weather_code],
                  hourly: %i[temperature_2m windspeed_10m relative_humidity_2m], daily: %i[temperature_2m_max temperature_2m_min weather_code] }
